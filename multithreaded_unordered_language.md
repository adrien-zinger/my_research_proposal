...
An inherently concurent and multithreaded language with adaptative ordonancements
with queueing theories applications.

# Abstract

This proposal contains an attempt to compille many design ideas of a general-purpose
programming language. If some sections may be a bit terse and not logically connected
to each other, it's because making a new language or thinking about what a good
programming language could look like is very hard. There is so much ways, branches,
choiced with no return possible in that kind of projects...

The main purpose of the research is to create a language that simply have inherently
concurrent and multithreading mechanisms. That is a basic idea, little bit weird because
all (many) programming languages already provide multithreading or concurrent systems,
and creating a programming language isn't so daunting if you know some of those 
books: *The dragon book*, *Crafting Interpretes*, *Introduction to Compilers and Language Design*.
Nevertheless, more we dig into a programming language specificity, and more we try to
make it performant, more it become chalenging.


This research primarily disign Eniem (name subject to change) based on a runtime with a
concurrent and optionally multithreaded approach. In Eniem classicals expressions
...
With that inovent paradigm, Eniem could enable runtime adaptation of the execution.
It includes job (expression) class specifications, dynamic reclassification. In addition,
Eniem toolchain provides a powerfull type checking and security features thanks to
an abstract interpreter that can be enabled which makes the code safe and correct.

# Pre introduction

Developping a programming language with a classical paradigm of ordonancement is already
a chalenge if you enable some features like recursions, dynamic arrays, type checking,
...
performant and how to implement the memory gestion, the mutability, et cetera.
...
In traditional programing language, the more recurrent paradigm is a strict
